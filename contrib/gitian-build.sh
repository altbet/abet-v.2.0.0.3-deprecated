...
VERSION=
commit=false
url=https://github.com/altbetproject/altbet
proc=2
mem=2000
lxc=true
...
Run this script from the directory containing the altbet, gitian-builder, gitian.sigs, and altbet-detached-sigs.
...
-u|--url	Specify the URL of the repository. Default is https://github.com/altbetproject/altbet
...
then
	# Make output folder
	mkdir -p "./altbet-binaries/${VERSION}"
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit altbet=${COMMIT} --url altbet=${url} ../altbet/contrib/gitian-descriptors/gitian-linux.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-linux --destination ../gitian.sigs/ ../altbet/contrib/gitian-descriptors/gitian-linux.yml
	    mv build/out/altbet-*.tar.gz build/out/src/altbet-*.tar.gz ../altbet-binaries/${VERSION}
	fi
	# Windows
	if [[ $windows = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit altbet=${COMMIT} --url altbet=${url} ../altbet/contrib/gitian-descriptors/gitian-win.yml
	    ...
	    mv build/out/altbet-*.zip build/out/altbet-*.exe ../altbet-binaries/${VERSION}
	fi
	# Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit altbet=${COMMIT} --url altbet=${url} ../altbet/contrib/gitian-descriptors/gitian-osx.yml
	    ...
	    mv build/out/altbet-*.tar.gz build/out/altbet-*.dmg ../altbet-binaries/${VERSION}
	fi
	# AArch64
	if [[ $aarch64 = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit altbet=${COMMIT} --url altbet=${url} ../altbet/contrib/gitian-descriptors/gitian-aarch64.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-aarch64 --destination ../gitian.sigs/ ../altbet/contrib/gitian-descriptors/gitian-aarch64.yml
	    mv build/out/altbet-*.tar.gz build/out/src/altbet-*.tar.gz ../altbet-binaries/${VERSION}
	fi
	popd || exit
...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../altbet/contrib/gitian-descriptors/gitian-win-signer.yml
	    ...
	    mv build/out/altbet-*win64-setup.exe ../altbet-binaries/${VERSION}
	    mv build/out/altbet-*win32-setup.exe ../altbet-binaries/${VERSION}
	fi
	# Sign Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../altbet/contrib/gitian-descriptors/gitian-osx-signer.yml
	    ...
	    mv build/out/altbet-osx-signed.dmg ../altbet-binaries/${VERSION}/altbet-${VERSION}-osx.dmg
	fi
	popd || exit
