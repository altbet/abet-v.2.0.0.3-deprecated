 ...
 *
 * This is the developer documentation of the reference client for an experimental new digital currency called Altbet (http://www.altbet.io),
 * which enables instant payments to anyone, anywhere in the world. Altbet uses peer-to-peer technology to operate
...
bool AppInit(int argc, char* argv[])
{
    ...
    // Parameters
    //
    // If Qt is used, parameters/altbet.conf are parsed in qt/altbet.cpp's main()
    ...
    try {
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "altbet:"))
                fCommandLine = true;
        ...
        if (fDaemon) {
            fprintf(stdout, "Altbet server starting\n");
