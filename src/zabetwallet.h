...
#ifndef ALTBET_ZABETWALLET_H
#define ALTBET_ZABETWALLET_H
...
public:
    CzABETWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZABET(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZABET(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //ALTBET_ZABETWALLET_H
